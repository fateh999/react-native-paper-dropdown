...
import { Checkbox, Divider, Menu, TextInput, TouchableRipple, useTheme, Caption, Text } from "react-native-paper";
import React, { forwardRef, useEffect, useState, useCallback, Fragment, } from "react";
const DropDown = forwardRef((props, ref) => {
    const activeTheme = useTheme();
    const { multiSelect = false, visible, disabled = false, onDismiss, showDropDown, value, setValue, activeColor, mode, label, placeholder, inputProps, list, dropDownContainerMaxHeight, dropDownContainerHeight, theme, dropDownStyle, dropDownItemStyle, dropDownItemSelectedStyle, dropDownItemTextStyle, dropDownItemSelectedTextStyle, accessibilityLabel, } = props;
    ...
    return (<Menu visible={visible && !disabled} onDismiss={onDismiss} theme={theme} anchor={<TouchableRipple ref={ref} onPress={disabled ? () => { } : showDropDown} onLayout={onLayout} accessibilityLabel={accessibilityLabel}>
            <View pointerEvents={"none"}>
              <TextInput value={displayValue} mode={mode} label={label} placeholder={placeholder} pointerEvents={"none"} theme={theme} disabled={disabled} right={<TextInput.Icon name={visible ? "menu-up" : "menu-down"}/>} {...inputProps}/>
            </View>
          ...
          {list.map((_item, _index) => (<Fragment key={_item.value}>
              <TouchableRipple style={{
                ...
                ...(isActive(_item.value)
                    ...
                    : dropDownItemTextStyle),
            }} title={<View style={styles.labelContainer}>
